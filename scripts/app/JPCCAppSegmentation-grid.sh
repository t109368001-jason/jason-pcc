...
for K in "${K_LIST[@]}"; do
  for ALPHA_N in "${ALPHA_N_LIST[@]}"; do
    ...
    ALPHA=${TMP_ARRAY[0]}
    N=${TMP_ARRAY[1]}
    for STATIC_THRESHOLD in "${STATIC_THRESHOLD_LIST[@]}"; do
      for DYNAMIC_THRESHOLD in "${DYNAMIC_THRESHOLD_LIST[@]}"; do
        OUTPUT_FOLDER=converted-grid/ZX-XS-20220707-ctc-gmm-segmentation[K-${K}][A-${ALPHA}-N-${N}][STGT-${STATIC_THRESHOLD}][DTLE-${DYNAMIC_THRESHOLD}]/
