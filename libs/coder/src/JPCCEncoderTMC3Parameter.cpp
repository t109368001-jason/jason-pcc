...
JPCCEncoderTMC3Parameter::JPCCEncoderTMC3Parameter(const string& prefix, const string& caption) :
  ...
  opts_.add_options()                                                     //
      ...
      ("bitwiseOccupancyCoding", value<bool>(&this->gps.bitwise_occupancy_coding_flag),  //
      ...
      ("jointTwoPointIdcm", value<bool>(&this->gps.joint_2pt_idcm_enabled_flag),                                    //
       "Jointly code common prefix of two IDCM points")                                                             //
      ("adjacentChildContextualization", value<bool>(&this->gps.adjacent_child_contextualization_enabled_flag),     //
      ...
      ("planarEnabled", value<bool>(&this->gps.geom_planar_mode_enabled_flag),                                      //
      ...
      ("planarModeIdcmUse",                                                                                         //
       ...
       "  0 => never, 32 => always")                                                        //
      ("trisoupNodeSizeLog2", value<vector<int>>(&this->trisoupNodeSizesLog2),              //
      ...
      ("trisoup_sampling_value", value<int>(&this->gps.trisoup_sampling_value),             //
       ...
       "  0: automatic")                                                                    //
      ("positionQuantisationEnabled", value<bool>(&this->gps.geom_scaling_enabled_flag),    //
       "Enable in-loop quantisation of positions")                                          //
      ("positionQuantisationMethod", value<OctreeEncOpts::QpMethod>(&this->geom.qpMethod),  //
      ...
      ("positionIdcmQp", value<int>(&this->idcmQp),                                         //
      ...
      ("positionSliceQpFreqLog2", value<int>(&this->gbh.geom_qp_offset_intvl_log2_delta),  //
       "Frequency of sending QP offsets in predictive geometry coding")                    //
      ("angularEnabled", value<bool>(&this->gps.geom_angular_mode_enabled_flag),           //
       "Controls angular contextualisation of occupancy")                                  //
      // NB: the underlying variable is in STV order.
      //     Conversion happens during argument sanitization.
      ("lidarHeadPosition", value<Vec3<int>>(&this->gps.gpsAngularOrigin),                          //
       "laser head position (x,y,z) in angular mode")                                               //
      ("numLasers", value<int>(&this->numLasers),                                                   //
       "Number of lasers in angular mode")                                                          //
      ("lasersTheta", value<vector<double>>(&this->lasersTheta),                                    //
       "Vertical laser angle in angular mode")                                                      //
      ("lasersZ", value<vector<double>>(&this->lasersZ), "Vertical laser offset in angular mode")   //
      ("lasersNumPhiPerTurn", value<vector<int>>(&this->gps.angularNumPhiPerTurn),                  //
       "Number of sampling poisitions in a complete laser turn in angular mode")                    //
      ("planarBufferDisabled", value<bool>(&this->gps.planar_buffer_disabled_flag),                 //
       "Disable planar buffer (when angular mode is enabled)")                                      //
      ("predGeomAzimuthQuantization", value<bool>(&this->gps.azimuth_scaling_enabled_flag),         //
       "Quantize azimuth according to radius in predictive geometry coding")                        //
      ("positionAzimuthScaleLog2", value<int>(&this->gps.geom_angular_azimuth_scale_log2_minus11),  //
       "Additional bits to represent azimuth angle in predictive geometry coding")                  //
      // NB: this will be corrected to be minus 1 later
      ("positionAzimuthSpeed", value<int>(&this->gps.geom_angular_azimuth_speed_minus1),         //
       "Scale factor applied to azimuth angle in predictive geometry coding")                    //
      ("positionRadiusInvScaleLog2", value<int>(&this->gps.geom_angular_radius_inv_scale_log2),  //
       "Inverse scale factor applied to radius in predictive geometry coding")                   //
      ("predGeomSort", value<PredGeomEncOpts::SortMode>(&this->predGeom.sortMode),               //
       "Predictive geometry tree construction order")                                            //
      ("predGeomAzimuthSortPrecision", value<float>(&this->predGeom.azimuthSortRecipBinWidth),   //
       "Reciprocal precision used in azimuthal sorting for tree construction")                   //
      ("predGeomTreePtsMax", value<int>(&this->predGeom.maxPtsPerTree),                          //
       "Maximum number of points per predictive geometry tree")                                  //
      ("pointCountMetadata", value<bool>(&this->gps.octree_point_count_list_present_flag),       //
       "Add octree layer point count metadata")                                                  //
      ("attrSphericalMaxLog2", value<int>(&this->attrSphericalMaxLog2),                          //
       "Override spherical coordinate normalisation factor")                                     //
      ("recolourSearchRange", value<int>(&this->recolour.searchRange), "")                       //
      ("recolourNumNeighboursFwd", value<int>(&this->recolour.numNeighboursFwd), "")             //
      ("recolourNumNeighboursBwd", value<int>(&this->recolour.numNeighboursBwd), "")             //
      ("recolourUseDistWeightedAvgFwd", value<bool>(&this->recolour.useDistWeightedAvgFwd), "")  //
      ("recolourUseDistWeightedAvgBwd", value<bool>(&this->recolour.useDistWeightedAvgBwd), "")  //
