...
DatasetParameter::DatasetParameter() : Parameter("DatasetOptions") {
  opts_.add_options()                                                                    //
      (TYPE_OPT, po::value<std::string>(&type), "dataset type")                          //
      (FOLDER_OPT, po::value<std::string>(&folder), "dataset folder")                    //
      (TOTAL_FILES_OPT, po::value<size_t>(&totalFiles), "dataset folder")                //
      (FILES_OPT, po::value<std::vector<std::string>>(&files), "dataset files")          //
      (FRAME_COUNTS_OPT, po::value<std::vector<size_t>>(&frameCounts), "dataset files")  //
      ;
}
...
void DatasetParameter::check() const {
  if (totalFiles != files.size()) { throw std::runtime_error(TOTAL_FILES_OPT " not match number of " FILES_OPT); }
  if (totalFiles != frameCounts.size()) {
    throw std::runtime_error(TOTAL_FILES_OPT " not match number of " FRAME_COUNTS_OPT);
  }
}

std::ostream& operator<<(std::ostream& out, const DatasetParameter& obj) {
  out << "DatasetParameter" << std::endl;
  out << "\t" TYPE_OPT "=" << obj.type << std::endl;
  out << "\t" FOLDER_OPT "=" << obj.folder << std::endl;
  out << "\t" TOTAL_FILES_OPT "=" << obj.totalFiles << std::endl;
  out << "\t" FILES_OPT "=";
  for (size_t i = 0; i < obj.files.size(); i++) {
    if (i == 0) { out << "["; }
    if (i != 0) { out << ", "; }
    out << obj.files[i];
    if (i == (obj.files.size() - 1)) { out << "]" << std::endl; }
  }
  ...
  for (size_t i = 0; i < obj.frameCounts.size(); i++) {
    ...
    if (i == (obj.frameCounts.size() - 1)) { out << "]" << std::endl; }
  }
  return out;
}
